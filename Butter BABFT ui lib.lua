    		...
    		for i,v in pairs(ScreenGui:GetChildren()) do
    		    spawn(function()
        		    if visible == true then
        			    v:TweenPosition(UDim2.new(0,v.Position.X.Offset,0,v.Position.Y.Offset - 500),Enum.EasingDirection.In,Enum.EasingStyle.Sine,0.5,true)
        			    wait(0.5)
        			    v.Visible = false
        			else
        			    v.Visible = true
    ...
    function Lib:CreateWindow(Name)
        ...
        ScrollHolder.ScrollBarImageColor3 = Color3.fromRGB(100, 100, 100)
        ScrollHolder.AutomaticCanvasSize = Enum.AutomaticSize.Y

        holder.Name = "holder"
        holder.Parent = ScrollHolder
        ...
        Minimize.MouseButton1Click:Connect(function()
            if MinimizeUsable == true then
                if Minimize.Text == "-" then
                    CurrentSize = Main.Size.Y.Offset
                    Minimize.Text = "+"
                    ...
                    Main:TweenSize(UDim2.new(0, Main.Size.X.Offset, 0, 25), Enum.EasingDirection.In, Enum.EasingStyle.Quad, 0.2, true)
                    wait(0.2)
                    MinimizeUsable = true;
                else
                    Minimize.Text = "-"
                    MinimizeUsable = false;
                    Main:TweenSize(UDim2.new(0, Main.Size.X.Offset, 0, CurrentSize), Enum.EasingDirection.In, Enum.EasingStyle.Quad, 0.2, true)
                    wait(0.15)
        ...
        -- Resize handle (positioned off the UI visually touching the bottom right)
        ...
        -- Resize drag logic (scales the entire UI proportionally)
        local MIN_SCALE = 0.5
        local MAX_SCALE = 3
        ...
        local function fadeHandle(targetBg, targetText)
            TweenService:Create(ResizeHandle, tweenInfo, {
        ...
        ResizeHandle.InputBegan:Connect(function(inp)
            if inp.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                local startScale = MainUIScale.Scale
                local startVisualW = Main.AbsoluteSize.X
                repeat
                    game:GetService("RunService").RenderStepped:Wait()
                    local dx = Mouse.X - startMouseX
                    local newScale = math.clamp(startScale * (startVisualW + dx) / startVisualW, MIN_SCALE, MAX_SCALE)
                    MainUIScale.Scale = newScale
                until inp.UserInputState == Enum.UserInputState.End
                IsDragging = false
            end
        end)
        ...
        ResizeHandle.MouseLeave:Connect(function()
            ...
            local cornerPos = mainPos + mainSize
            local mousePos = Vector2.new(Mouse.X, Mouse.Y)
            local dist = (cornerPos - mousePos).Magnitude
            if dist < 60 or IsDragging then
                fadeHandle(0.5, 0.5)
            else
                fadeHandle(1, 1)
            end
        ...
        game:GetService("RunService").RenderStepped:Connect(function()
            if IsDragging or IsHovering then
                wasClose = true
                return
            end
            local mainPos = Main.AbsolutePosition
            ...
            local mousePos = Vector2.new(Mouse.X, Mouse.Y)
            local dist = (cornerPos - mousePos).Magnitude
            local isClose = (dist < 60)
            if isClose ~= wasClose then
                wasClose = isClose
                if isClose then
                    fadeHandle(0.5, 0.5)
                else
        ...
        function Window:Slider(name, options, callback)
            ...
            local function MakeChange()
                ...
                spawn(function()
                    repeat game:GetService("RunService").RenderStepped:wait()
                        Fill.Size = UDim2.new(0,(game.Players.LocalPlayer:GetMouse().X - Fill.AbsolutePosition.X + 3) / MainUIScale.Scale,0,6)
                    until MouseDown == false
                    Check:Disconnect()
                end)
                Check = Fill.Changed:Connect(function()
                    local fillLogicalW = Fill.AbsoluteSize.X / MainUIScale.Scale
                    if precise then
                        location[flag] = min + math.ceil(((max - min)/134) * (fillLogicalW - 6))
                        Text_6.Text = tostring(location[flag])
                    else
                        location[flag] = min + roundDecimals(((max - min)/134) * (fillLogicalW - 6),2)
            ...
            Text.MouseButton1Click:Connect(function()
                if DropDownUsable == true then
                    if TextButton.Rotation == 0 then
                        ...
                        Main:TweenSize(UDim2.new(0, Main.Size.X.Offset, 0, Main.Size.Y.Offset + 100), Enum.EasingDirection.In, Enum.EasingStyle.Quad, 0.2, true)
                        wait(0.2)
                        DropDownUsable = true
                        Del(true)
    ...
    function Window:ImageDropdown(name, options, callback)
    local location   = options.location or self.flags
    local flag       = options.flag or ""
    local callback   = callback or function() end
    local Search     = options.search or false
    ...
    local function FixCanvas()
        ...
        ScrollingFrame.CanvasSize = UDim2.new(0,0,0,Amount * 45)
    end

    Dropdown.Name = "ImageDropdown"
    Dropdown.Parent = holder
    Dropdown.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    Dropdown.BackgroundTransparency = 1.000
    Dropdown.Size = UDim2.new(1, 0, 0, 25)
    ...
    DropdownDisplay.Parent = Text
    DropdownDisplay.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    DropdownDisplay.BackgroundTransparency = 1.000
    DropdownDisplay.Size = UDim2.new(1, -30, 1, 0)
    
    DisplayImage.Name = "Image"
    DisplayImage.Parent = DropdownDisplay
    DisplayImage.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    DisplayImage.BackgroundTransparency = 1.000
    DisplayImage.Position = UDim2.new(0, 0, 0, 0)
    DisplayImage.Size = UDim2.new(0, 20, 1, 0)
    DisplayImage.Image = ""
    DisplayImage.ScaleType = Enum.ScaleType.Fit

    DisplayText.Name = "Text"
    DisplayText.Parent = DropdownDisplay
    DisplayText.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    DisplayText.BackgroundTransparency = 1.000
    DisplayText.Position = UDim2.new(0, 25, 0, 0)
    ...
    DisplayText.TextSize = 12.000
    DisplayText.TextStrokeColor3 = Color3.fromRGB(255, 255, 255)
    DisplayText.TextStrokeTransparency = 0.920
    DisplayText.TextXAlignment = Enum.TextXAlignment.Left
    DisplayText.Text = options.list[1].name or tostring(options.list[1])

    -- Set initial display image
    if options.list[1].image then
        DisplayImage.Image = options.list[1].image
    end
    ...
    Text.MouseButton1Click:Connect(function()
        if DropDownUsable == true then
            if TextButton.Rotation == 0 then
                ...
                Holder.Size = UDim2.new(0, 140, 0, 1) 
                DropDownUsable = false
                Holder:TweenSize(UDim2.new(0, 140, 0, 200), Enum.EasingDirection.In, Enum.EasingStyle.Quad, 0.2, true)
                Main:TweenSize(UDim2.new(0, Main.Size.X.Offset, 0, Main.Size.Y.Offset + 200), Enum.EasingDirection.In, Enum.EasingStyle.Quad, 0.2, true)
                wait(0.2)
                DropDownUsable = true
                Del(true)
                if DropdownType == "Update" then
                ...
                -- Create dropdown items with images
                for i,v in pairs(options.list) do
                    local ItemFrame = Instance.new("Frame")
                    local ItemImage = Instance.new("ImageLabel")
                    local ItemText = Instance.new("TextButton")
                    
                    ItemFrame.Name = tostring(v.name or v)
                    ItemFrame.Parent = ScrollingFrame
                    ItemFrame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
                    ItemFrame.BackgroundTransparency = 1.000
                    ItemFrame.Size = UDim2.new(0, 132, 0, 40)
                    ...
                    ItemImage.Position = UDim2.new(0, 5, 0.125, 0)
                    ItemImage.Size = UDim2.new(0, 30, 0, 30)
                    ItemImage.Image = v.image or ""
                    ...
                    ItemText.Size = UDim2.new(0, 85, 1, 0)
                    ItemText.Font = Enum.Font.GothamBold
                    ItemText.Text = "  " .. (v.name or tostring(v))
                    ItemText.TextColor3 = Color3.fromRGB(255, 255, 255)
                    ItemText.TextSize = 11.000
                    ItemText.TextStrokeColor3 = Color3.fromRGB(255, 255, 255)
                    ItemText.TextStrokeTransparency = 0.920
                    ItemText.TextXAlignment = Enum.TextXAlignment.Left
                    ItemText.ZIndex = 3
                    
                    ItemText.MouseButton1Click:Connect(function()
                        -- Update selected item styling
                        for i,v in pairs(ScrollingFrame:GetChildren()) do
                            if v:IsA("Frame") and v:FindFirstChild("Text") then
                                v:FindFirstChild("Text").TextStrokeColor3 = Color3.fromRGB(255, 255, 255)
                            end
                        end
                        ItemText.TextStrokeColor3 = Color3.fromRGB(200, 200, 200)
                        
                        -- Update location with the selected item
                        location[flag] = v
                        
                        -- Update display
                        DisplayText.Text = v.name or tostring(v)
                        if v.image then
                            DisplayImage.Image = v.image
                        end
    ...
    ScrollingFrame.BackgroundTransparency = 1.000
    ScrollingFrame.BorderSizePixel = 0
    ScrollingFrame.Size = UDim2.new(0, 137, 0, 200)
    ScrollingFrame.ScrollBarThickness = 4
    ScrollingFrame.ZIndex = 3
    ScrollingFrame.CanvasSize = UDim2.new(0,0,0,0)
    ...
    TextBox:GetPropertyChangedSignal("Text"):Connect(function()
        for i,v in pairs(ScrollingFrame:GetChildren()) do
            if v:IsA("Frame") then
                local itemText = v:FindFirstChild("Text")
                if itemText then
                    if string.match(itemText.Text:lower(), TextBox.Text:lower()) then
                        v.Visible = true
                        FixCanvas()
                    else
                        v.Visible = false
            ...
            minimize.MouseButton1Click:Connect(function()
                ...
                if Minimize2Usable == true then
                    if minimize.Text == "-" then
                        ...
                        Frame:TweenSize(UDim2.new(0, Main.Size.X.Offset, 0, 0), Enum.EasingDirection.In, Enum.EasingStyle.Quad, 0.2, true)
                        Main:TweenSize(UDim2.new(0, Main.Size.X.Offset, 0, Main.Size.Y.Offset - Numbers), Enum.EasingDirection.In, Enum.EasingStyle.Quad, 0.2, true)
                        wait(0.2)
                        Minimize2Usable = true;
                        Frame:Destroy()
                    else
